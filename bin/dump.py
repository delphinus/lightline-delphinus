...
def nord_improved() -> None:
    ...
    script = f'call writefile([{func}()], "{filename}")'
    run(f"vim +'{script}' +q", shell=True)
    colors = filename.read_text().strip()
    filename.write_text(
        dedent(
            f"""
            " DO NOT EDIT THIS FILE
            " generated by bin/dump.py
            let g:lightline#colorscheme#nord_improved#palette={colors}
            if g:nord_uniform_status_lines
              let g:lightline#colorscheme#nord_improved#palette.inactive.middle[0][1]='{NORD1}'
            endif
            """
        )
    )
